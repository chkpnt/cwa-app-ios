...
protocol AppConfigurationProviding: AnyObject {
	typealias Completion = (Result<Cwa_Internal_V2_ApplicationConfigurationIOS, Error>) -> Void
...
struct AppConfigurationFetchingResponse {
	let config: Cwa_Internal_V2_ApplicationConfigurationIOS
	let eTag: String?

	init(_ config: Cwa_Internal_V2_ApplicationConfigurationIOS, _ eTag: String? = nil) {
		self.config = config
		self.eTag = eTag
	}
}
