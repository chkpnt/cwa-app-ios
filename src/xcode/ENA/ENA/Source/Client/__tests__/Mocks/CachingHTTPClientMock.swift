...
final class CachingHTTPClientMock: CachingHTTPClient {
	...
	static let staticAppConfig: Cwa_Internal_V2_ApplicationConfigurationIOS = {
		...
		guard
			...
			let config = try? Cwa_Internal_V2_ApplicationConfigurationIOS(serializedData: bucket.bin)
		else {
			assertionFailure("check this!")
			return Cwa_Internal_V2_ApplicationConfigurationIOS()
		}
		return config
	}()
