...
class EUSettingsViewControllerTests: XCTestCase {
	...
	func testDataReloadForSuccessfulDownload() {
		var config = Cwa_Internal_V2_ApplicationConfigurationIOS()
		config.supportedCountries = ["DE", "ES", "FR", "IT", "IE", "DK"]
		let fakeResult: (Result<Cwa_Internal_V2_ApplicationConfigurationIOS, Error>) = .success(config)
	...
	func testDataReloadForUnsuccessfulDownload() {
		let fakeResult: (Result<Cwa_Internal_V2_ApplicationConfigurationIOS, Error>) = .failure(URLSession.Response.Failure.noResponse)
		let appConfigurationProviderStub = AppConfigurationProviderStub(result: fakeResult)
		let vc = EUSettingsViewController(appConfigurationProvider: appConfigurationProviderStub)
		vc.viewDidLoad()
...
private class AppConfigurationProviderStub: AppConfigurationProviding {
	...
	let result: (Result<Cwa_Internal_V2_ApplicationConfigurationIOS, Error>)

	init(result: (Result<Cwa_Internal_V2_ApplicationConfigurationIOS, Error>)) {
		self.result = result
	}
