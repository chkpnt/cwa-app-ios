...
extension SecureStore: AppConfigCaching {
	...
	var appConfig: Cwa_Internal_V2_ApplicationConfigurationIOS? {
		get {
			guard let data = kvStore["Cwa_Internal_V2_ApplicationConfigurationIOS"] else { return nil }
			return try? Cwa_Internal_V2_ApplicationConfigurationIOS(serializedData: data)
		}
		set { kvStore["Cwa_Internal_V2_ApplicationConfigurationIOS"] = try? newValue?.serializedData() }
	}
}
